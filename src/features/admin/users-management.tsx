...
export function UsersManagement() {
  ...
  const brands = allUsers.filter((user) => user.role === "brand")
  ...
  const getSubscriptionBadge = (subscription: string) => {
    ...
    switch (subscription) {
      case "basic":
        return <Badge variant="secondary" className={`${base}`}>Basic</Badge>
      case "premium":
      ...
      default:
        return <Badge variant="secondary" className={`${base}`}>{subscription}</Badge>
    }
  }
  ...
  const UserTable = ({ users, userType }: { users: any[]; userType: string }) => (
    <Table>
      <TableHeader>
        <TableRow>
          ...
          {userType === "influencer" && <TableHead>Followers</TableHead>}
      ...
      <TableBody>
        {users.map((user) => (
          <TableRow key={user.id}>
            ...
            {userType === "influencer" && <TableCell>{user.followers.toLocaleString()}</TableCell>}
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="influencers" className="w-full">
        ...
        <TabsContent value="influencers" className="mt-6">
          <Card>
            <CardHeader>
              <CardTitle>Danh sách Influencers</CardTitle>
              <CardDescription>Tổng cộng {influencers.length} influencers</CardDescription>
            </CardHeader>
            <CardContent>
              <UserTable users={filterUsers(influencers)} userType="influencer" />
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="brands" className="mt-6">
          <Card>
            <CardHeader>
              <CardTitle>Danh sách Brands</CardTitle>
              <CardDescription>Tổng cộng {brands.length} brands</CardDescription>
            </CardHeader>
            <CardContent>
              <UserTable users={filterUsers(brands)} userType="brand" />
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2 mt-2">
                    <Badge variant="outline">{selectedUser.role === "influencer" ? "Influencer" : "Brand"}</Badge>
