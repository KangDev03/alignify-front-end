...
export default function CampaignDetailPage() {
  ...
  useEffect(() => {
    if (campaign?.campaignRequirements) {
      setSelectRequirement(initializeSelectRequirement(campaign.campaignRequirements));
    }
            ...
            {orderedCampaignRequirements.map((require: any, idx: number) => {
              ...
              return (
                <Badge
                  ...
                  variant="outline"
                  className={`lowercase text-white ${style}`}
                >
                  <Icon className={platform === 'tiktok' ? 'stroke-3' : ''} />
